...
public class QuickEntryActivity extends BridgeActivity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        int height = Math.min((int) (490 * density), (int) (getResources().getDisplayMetrics().heightPixels * .85));
        getWindow().setLayout(width, height);
        getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
        setFinishOnTouchOutside(false);
        if (bridge != null) bridge.getWebView().setBackgroundColor(Color.TRANSPARENT);
    }
}
