...
/* Empty states are the one place people read carefully, because there is
   nothing else on screen. Each names its feature and what it actually buys
  ...
  return (
    <div className="vault-screen pl-mobile-plan">
      ...
      <section className="pl-hero" aria-label="Limit money left">
        ...
        <strong className="pl-hero-figure vault-digits">{money(remaining)}</strong>
        <p className="pl-hero-sub">{money(totalUsed)} of {money(totalLimit)} used across your {budgets.length} {budgets.length === 1 ? 'limit' : 'limits'}</p>
      ...
      {sections.map(({ key }) => (
        <section key={key} ref={(node) => { sectionRefs.current[key] = node }} data-section={key} className="pl-mobile-section scroll-mt-16">
          ...
          <div className="pl-mobile-list">
            ...
            {countFor(key) === 0 && <button className="pl-mobile-empty" type="button" onClick={() => addFor(key)}><Plus size={18} /><span><strong>{sectionEmpty[key].title}</strong><small>{sectionEmpty[key].body}</small></span></button>}
          </div>
...
function MobilePlanRow({ kind, item, transactions, onOpen }: { kind: PlanSection; item: Budget | UpcomingExpense | WishlistItem | MoneyQuest; transactions: PlanData['transactions']; onOpen: () => void }) {
  ...
  if (kind === 'limits') {
    ...
    return <button className="pl-mobile-row" type="button" onClick={onOpen}><span className="pl-row-icon is-clay"><CategoryIcon label={budget.category} type="expense" size={18} /></span><span><strong>{budget.category}</strong><small>{money(Math.max(0, budget.amount - budget.used))} left · {used}% used</small><i><b className={used >= 80 ? 'is-hot' : ''} style={{ width: `${Math.min(100, used)}%` }} /></i></span><span className="vault-digits">{money(budget.amount)}</span></button>
  }
  if (kind === 'bills') {
    const bill = item as UpcomingExpense
    const days = daysUntil(bill.dueDate)
    return <button className="pl-mobile-row" type="button" onClick={onOpen}><span className="pl-row-icon is-sage"><CalendarDays size={18} /></span><span><strong>{bill.title}</strong><small>{days < 0 ? `${Math.abs(days)} days overdue` : `Due ${formatPlanDate(bill.dueDate)}`} · {bill.isRecurring ? bill.recurringFrequency?.replace('_', ' ') : 'one-time'}</small></span><span className="vault-digits">{money(bill.amount)}</span></button>
  }
  if (kind === 'cooling') {
    const wish = item as WishlistItem
    const ready = wish.status === 'ready' || new Date(wish.reconsiderAt).getTime() <= now
    return <button className="pl-mobile-row" type="button" onClick={onOpen}><span className="pl-row-icon is-taupe"><Clock3 size={18} /></span><span><strong>{wish.name}</strong><small>{ready ? <b className="pl-row-ready">Ready to decide</b> : `Think until ${formatPlanDate(wish.reconsiderAt)}`}{wish.reason ? ` · ${wish.reason}` : ''}</small></span><span className="vault-digits">{money(wish.amount)}</span></button>
  }
...
function QuestSheet({ categories, goals, onClose, onSave }: { categories: PlanData['categories']; goals: PlanData['goals']; onClose: () => void; onSave: (quest: MoneyQuest) => void }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    const title = type === 'tracking_days' ? `Track money on ${value} days` : type === 'no_spend_days' ? `${value} no-spend days` : type === 'goal_contribution' ? `Add ${money(value)} to ${goal?.name ?? 'a goal'}` : `Keep ${category?.name ?? 'category'} under ${money(value)}`
...
function DetailSheet({ target, accounts, categories, goals, transactions, onClose, onEdit, onArchiveLimit, onPayBill, onCancelBill, onSaveWishlist, onRemoveWishlist, onBuyWishlist, onEndQuest, onRepeatQuest }: { target: DetailTarget; accounts: PlanData['accounts']; categories: PlanData['categories']; goals: PlanData['goals']; transactions: PlanData['transactions']; onClose: () => void; onEdit: () => void; onArchiveLimit: (budget: Budget) => void; onPayBill: (bill: UpcomingExpense) => void; onCancelBill: (bill: UpcomingExpense) => void; onSaveWishlist: (item: WishlistItem) => void; onRemoveWishlist: (item: WishlistItem) => void; onBuyWishlist: (item: WishlistItem) => void; onEndQuest: (quest: MoneyQuest) => void; onRepeatQuest: (quest: MoneyQuest) => void }) {
  ...
  if (target.kind === 'limit') {
    ...
    return <PlanDetailShell eyebrow="Spending limit" accent="#C85A2A" title={target.item.category} hero={<span className="pl-hero-fig vault-digits">{money(remaining)} <small>left</small></span>} pill={<span className="pl-hero-pill-tag is-clay">{used}% used</span>} onClose={onClose}>
      <span className="pl-usage-bar"><i style={{ width: `${Math.min(100, used)}%` }} /></span>
      <PlanDetailList items={[['Monthly limit', `${money(target.item.amount)}`], ['Spent so far', `${money(spent)}`], ['Cycle', cycle]]} />
      <p className="pl-pace-note">{used >= 80 && daysAtPace ? <>You’re close to the ceiling. <strong>{money(remaining)}</strong> covers about {daysAtPace} more {daysAtPace === 1 ? 'day' : 'days'} at your current pace.</> : <><strong>{money(remaining)}</strong> of your {money(target.item.amount)} limit is still free this month.</>}</p>
  ...
  if (target.kind === 'bill') {
    ...
    return <PlanDetailShell eyebrow="Scheduled bill" accent="#657355" title={target.item.title} hero={<span className="pl-hero-fig vault-digits">{money(target.item.amount)}</span>} pill={<span className="pl-hero-pill-tag is-sage">Due {formatPlanDate(target.item.dueDate, true)}</span>} onClose={onClose}>
  ...
  if (target.kind === 'cooling') {
    ...
    return <PlanDetailShell eyebrow="Cool-off decision" accent="#837661" title={item.name} hero={<span className="pl-hero-fig vault-digits">{money(item.amount)}</span>} pill={<span className="pl-hero-pill-tag is-taupe">{ready ? 'Ready to decide' : 'Cooling off'}</span>} onClose={onClose}>
...
function HistorySheet({ budgets, bills, filter, onFilter, onClose, onRestoreBudget }: { budgets: Budget[]; bills: UpcomingExpense[]; wishlist: WishlistItem[]; quests: MoneyQuest[]; filter: PlanHistoryFilter; onFilter: (filter: PlanHistoryFilter) => void; onClose: () => void; onRestoreBudget: (budget: Budget) => void; onRepeatQuest: (quest: MoneyQuest) => void }) {
  ...
  return <VaultSheet open label="Plan history" onClose={onClose}><div className="pl-sheet-head"><div><p className="vault-eyebrow">Past planning</p><h2 className="vault-sheet-title text-left">Plan history</h2></div></div><div className="vault-chiprow mt-4 overflow-x-auto">{(['all', 'limits', 'bills'] as PlanHistoryFilter[]).map((item) => <button key={item} className={cn('vault-chip', filter === item && 'is-active')} type="button" onClick={() => onFilter(item)}>{item === 'all' ? 'All' : item === 'cooling' ? 'Cool-off' : item[0].toUpperCase() + item.slice(1)}</button>)}</div><div className="pl-history-list">{entries.map((entry) => <div key={entry.id}><span className="pl-row-icon"><History size={16} /></span><p><strong>{entry.title}</strong><small>{entry.meta}</small></p>{entry.action && <button type="button" onClick={entry.action}>{entry.actionLabel}</button>}</div>)}{entries.length === 0 && <p className="pl-empty-copy">Nothing in this part of your history yet.</p>}</div></VaultSheet>
}
