...
function QuestPanel({ activeQuest, categories, goals, transactions, onSave, onCancel }: { activeQuest?: MoneyQuest; categories: Category[]; goals: Goal[]; transactions: Transaction[]; onSave: (quest: MoneyQuest) => void; onCancel: (quest: MoneyQuest) => void }) {
  ...
  const create = (event: FormEvent) => { event.preventDefault(); const start = new Date(); const end = new Date(); end.setDate(end.getDate() + 6); const value = Math.max(1, Number(target)); const title = type === 'tracking_days' ? `Track money on ${value} days` : type === 'no_spend_days' ? `${value} no-spend days` : type === 'category_limit' ? `Keep category spending under ${formatPKR(value)}` : `Put ${formatPKR(value)} toward a goal`; onSave({ id: crypto.randomUUID(), type, title, categoryId: type === 'category_limit' ? categoryId : undefined, goalId: type === 'goal_contribution' ? goalId : undefined, targetCount: type === 'tracking_days' || type === 'no_spend_days' ? value : undefined, targetAmount: type === 'category_limit' || type === 'goal_contribution' ? value : undefined, startsOn: start.toISOString().slice(0, 10), endsOn: end.toISOString().slice(0, 10), status: 'active' }); trackEvent('quest_started', { surface: 'plan' }) }
