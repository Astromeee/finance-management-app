...
export function Settings(props: Props) {
  ...
  const addCategory = async (event: FormEvent) => {
    ...
    await props.onSaveCategory({ id: crypto.randomUUID(), name, kind: categoryKind, color: categoryKind === 'income' ? '#E79564' : '#F66924', spendingNature: categoryKind === 'income' ? 'flexible' : categoryNature })
    setCategoryName('')
...
function SettingsRow({ title, detail, icon: Icon, trailing }: { title: string; detail: string; icon: typeof Moon; trailing?: ReactNode }) {
  return <div className="flex items-center gap-4 border-b border-[var(--border)] px-4 py-3.5 last:border-b-0"><span className="grid h-10 w-10 flex-none place-items-center rounded-[14px] border border-[rgba(246,105,36,.22)] bg-[var(--accent-soft)] text-[var(--accent)]"><Icon size={18} /></span><div className="min-w-0 flex-1"><h3 className="text-[15px] font-semibold text-white">{title}</h3><p className="mt-0.5 text-[13px] text-[var(--muted)]">{detail}</p></div>{trailing}</div>
}
