    ...
    <main className="auth-shell grid min-h-screen place-items-center p-5">
      <section className="auth-card w-full max-w-md rounded-[2rem] p-6">
        ...
        <form className="grid gap-4" onSubmit={submit}>
          ...
          {message && <p className={`rounded-2xl border p-3 text-sm ${success ? 'border-[rgba(246,105,36,.28)] bg-[rgba(246,105,36,.13)] text-[var(--positive)]' : 'border-[rgba(232,105,74,.25)] bg-[rgba(232,105,74,.08)] text-[var(--negative)]'}`} role="status">{success && <CheckCircle2 className="mr-2 inline" size={16} />}{message}</p>}
        </form>
