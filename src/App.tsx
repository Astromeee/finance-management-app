...
function App() {
  ...
  const pages: Record<string, { title: string; subtitle: string; component: ReactNode }> = {
    ...
    categories: { title: 'Categories', subtitle: 'Manage your categories', component: <Categories categories={categories} transactions={transactions} onNavigate={setActivePage} onSaveCategory={async (category) => { await saveCategory(category); setCategories((current) => [...current.filter((item) => item.id !== category.id), category]) }} onArchiveCategory={async (id) => { await archiveCategory(id); setCategories((current) => current.filter((item) => item.id !== id)) }} /> },
