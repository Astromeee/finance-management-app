  ...
  return <div className="d-modal-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <aside className="d-slide-over gd-detail-panel" role="dialog" aria-modal="true" aria-labelledby="goal-detail-title">
      ...
      <div className="d-modal-body">
        <section className="gd-panel-progress"><div><span>{progress}% complete</span><strong>Rs {nf(goal.saved)} <small>of Rs {nf(goal.target)}</small></strong></div><i><b style={{ width: `${progress}%` }}/></i><p>Rs {nf(remaining)} remaining</p></section>
        <dl className="gd-detail-grid"><div><dt>Target date</dt><dd>{formatDate(goal.dueDate)}</dd></div><div><dt>Pace</dt><dd><PaceBadge pace={goalPace(goal)}/></dd></div><div><dt>Monthly needed</dt><dd>Rs {nf(monthlyContributionNeeded(goal))}</dd></div><div><dt>Linked account</dt><dd>{account?.name ?? 'Not linked'}</dd></div></dl>
...
function DebtDetailPanel({ debt, close, onPay, onEdit, onDelete }: { debt: Debt; close: () => void; onPay: () => void; onEdit: () => void; onDelete: () => void }) {
  ...
  return <div className="d-modal-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <aside className="d-slide-over gd-detail-panel" role="dialog" aria-modal="true" aria-labelledby="debt-detail-title">
      ...
      <div className="d-modal-body">
        <section className="gd-panel-progress"><div><span>{progress}% repaid</span><strong>Rs {nf(paid)} <small>of Rs {nf(total)}</small></strong></div><i><b style={{ width: `${progress}%` }}/></i><p>Rs {nf(remaining)} outstanding</p></section>
    ...
    <div className="gd-workspace">
      <div className="gd-primary-column">
        <section className="gd-hero">
          {featured ? <>
            <div className="gd-hero-top"><div><span className="gd-eyebrow">Featured path</span><div className="gd-feature-title"><span><Target size={21}/></span><h2>{featured.name}</h2></div><p><strong>Rs {nf(featured.saved)}</strong> of Rs {nf(featured.target)} saved</p></div><div className="gd-hero-ring" style={{ background: `conic-gradient(#E2703A ${featuredProgress}%,#4A4035 0)` }}><span>{featuredProgress}<small>%</small></span></div></div>
            <div className="gd-hero-progress"><i><b style={{ width: `${featuredProgress}%` }}/></i><div><strong>Rs {nf(Math.max(0, featured.target - featured.saved))} to go</strong><span>{featuredProgress}%</span></div></div>
            <div className="gd-hero-stats"><div><span>Target date</span><strong>{formatDate(featured.dueDate)}</strong></div><div><span>Pace</span><strong>{goalPace(featured)}</strong></div><div><span>Monthly needed</span><strong>Rs {nf(monthlyContributionNeeded(featured))}</strong></div></div>
        ...
        <section className="gd-card gd-goals-list">
          <header><div><span className="gd-eyebrow">Your progress</span><h2>All goals</h2></div><p><strong>Rs {nf(totalSaved)}</strong> saved of Rs {nf(totalTarget)}</p></header>
          ...
          {goals.map((goal) => { const progress = goalProgress(goal); return <button type="button" className="gd-goal-row" key={goal.id} onClick={() => setDetail({ kind: 'goal', id: goal.id })}>
            <span className="gd-row-icon"><Target size={18}/></span><span className="gd-row-title"><strong>{goal.name}</strong><small>Rs {nf(goal.saved)} of Rs {nf(goal.target)}</small></span><span className="gd-row-progress"><i><b style={{ width: `${progress}%` }}/></i><small>{progress}% funded</small></span><span className="gd-row-amount">Rs {nf(Math.max(0, goal.target - goal.saved))}</span><span className="gd-row-status"><small>{formatDate(goal.dueDate)}</small><PaceBadge pace={goalPace(goal)}/></span><ChevronRight size={16}/>
          </button> })}
