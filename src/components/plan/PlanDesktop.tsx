...
function LimitRow({ budget, onClick }: { budget: Budget; onClick: () => void }) { const progress = Math.round(budget.used / Math.max(1, budget.amount) * 100); return <button className="pl-desktop-row" type="button" onClick={onClick}><span className="pl-desktop-row-icon"><CategoryIcon label={budget.category} type="expense" size={18} /></span><p><strong>{budget.category}</strong><small>{money(Math.max(0, budget.amount - budget.used))} remaining</small></p><span className="pl-desktop-progress"><i><b className={progress >= 80 ? 'is-hot' : ''} style={{ width: `${Math.min(100, progress)}%` }} /></i><small>{progress}% used</small></span><strong className="pl-row-money">{money(budget.used)} <small>/ {nf(budget.amount)}</small></strong><ChevronRight size={16} /></button> }
function BillRow({ bill, onClick, onPay }: { bill: UpcomingExpense; onClick: () => void; onPay: () => void }) { const days = daysUntil(bill.dueDate); return <div className="pl-desktop-row"><span className="pl-desktop-row-icon is-sage"><CalendarDays size={18} /></span><button className="pl-row-main" type="button" onClick={onClick}><strong>{bill.title}</strong><small>{days < 0 ? `${Math.abs(days)} days overdue` : `Due ${formatPlanDate(bill.dueDate)}`} · {bill.isRecurring ? bill.recurringFrequency?.replace('_', ' ') : 'one-time'}</small></button><span>{bill.category}</span><strong className="pl-row-money">{money(bill.amount)}</strong><button className="pl-row-action" type="button" onClick={onPay}>Pay</button><button className="pl-row-chevron" type="button" aria-label={`View ${bill.title}`} onClick={onClick}><ChevronRight size={16} /></button></div> }
function CoolingRow({ item, onClick }: { item: WishlistItem; onClick: () => void }) { const [now] = useState(() => Date.now()); const ready = item.status === 'ready' || new Date(item.reconsiderAt).getTime() <= now; return <button className="pl-desktop-row" type="button" onClick={onClick}><span className="pl-desktop-row-icon is-taupe"><Clock3 size={18} /></span><p><strong>{item.name}</strong><small>{item.reason || 'No note added'}</small></p><span className={`pl-status ${ready ? 'is-ready' : ''}`}>{ready ? 'Ready' : `Until ${formatPlanDate(item.reconsiderAt)}`}</span><strong className="pl-row-money">{money(item.amount)}</strong><ChevronRight size={16} /></button> }
...
function DesktopHistory({ budgets, bills, filter, onFilter, onRestoreBudget }: { budgets: Budget[]; bills: UpcomingExpense[]; wishlist: WishlistItem[]; quests: MoneyQuest[]; filter: PlanHistoryFilter; onFilter: (filter: PlanHistoryFilter) => void; onRestoreBudget: (budget: Budget) => void; onRepeatQuest: (quest: MoneyQuest) => void }) {
  const rows: Array<{ kind: PlanSection; id: string; title: string; status: string; detail: string; date: string; action?: () => void; actionLabel?: string }> = [
    ...budgets.map((item) => ({ kind: 'limits' as const, id: `budget-${item.id}`, title: item.category, status: item.archived ? 'Archived' : 'Past month', detail: `${money(item.amount)} · ${item.periodMonth?.slice(0, 7)}`, date: item.updatedAt ?? item.periodMonth ?? '', action: item.archived ? () => onRestoreBudget(item) : undefined, actionLabel: 'Restore' })),
    ...bills.filter((item) => !isBillActive(item)).map((item) => ({ kind: 'bills' as const, id: `bill-${item.id}`, title: item.title, status: item.status, detail: `${money(item.amount)} · ${formatPlanDate(item.dueDate)}`, date: item.dueDate })),
  ].filter((item) => filter === 'all' || item.kind === filter).sort((a, b) => b.date.localeCompare(a.date))
  return <section className="pl-desktop-card pl-manager pl-history-manager"><header><div><p>{rows.length} RECORDS</p><h2>Plan history</h2><small>Restore a limit or repeat a useful challenge without losing the original record.</small></div></header><div className="pl-desktop-history-filters">{(['all', 'limits', 'bills'] as PlanHistoryFilter[]).map((item) => <button key={item} className={filter === item ? 'is-active' : ''} type="button" onClick={() => onFilter(item)}>{item === 'cooling' ? 'Cool-off' : item[0].toUpperCase() + item.slice(1)}</button>)}</div><div className="pl-manager-list">{rows.map((row) => <div className="pl-desktop-row" key={row.id}><span className="pl-desktop-row-icon"><History size={17} /></span><p><strong>{row.title}</strong><small>{row.detail}</small></p><span className="pl-status">{row.status}</span>{row.action && <button className="pl-row-action" type="button" onClick={row.action}>{row.actionLabel}</button>}</div>)}{rows.length === 0 && <Empty icon={History} title="No history here yet" detail="Completed and archived planning items will appear here." />}</div></section>
}
...
function DesktopLimitPanel({ panel, data, actions, onClose, onPanel }: { panel: Extract<Panel, { kind: 'limit' }>; data: PlanData; actions: PlanActions; onClose: () => void; onPanel: (panel: Panel) => void }) {
  ...
  return <form className="pl-panel-form" onSubmit={submit}><DesktopField label="Category"><select disabled={Boolean(item)} value={categoryId} onChange={(event) => setCategoryId(event.target.value)}>{available.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></DesktopField><DesktopField label="Monthly limit"><span>{currencySymbol()}</span><input min="1" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} /></DesktopField><p className="pl-panel-note">Editing the amount never resets the spending calculated from your ledger.</p><button className="d-button is-primary" disabled={!categoryId || Number(amount) <= 0}>Save limit</button></form>
}
...
function DesktopQuestPanel({ panel, data, actions, onClose }: { panel: Extract<Panel, { kind: 'quest' }>; data: PlanData; actions: PlanActions; onClose: () => void }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); const value = Math.max(1, Number(target)); const category = data.categories.find((entry) => entry.id === categoryId); const goal = data.goals.find((entry) => entry.id === goalId); const ends = new Date(); ends.setDate(ends.getDate() + 6); actions.onSaveQuest({ id: crypto.randomUUID(), type, title: type === 'no_spend_days' ? `${value} no-spend days` : type === 'tracking_days' ? `Track money on ${value} days` : type === 'category_limit' ? `Keep ${category?.name ?? 'category'} under ${money(value)}` : `Add ${money(value)} to ${goal?.name ?? 'a goal'}`, categoryId: type === 'category_limit' ? categoryId : undefined, goalId: type === 'goal_contribution' ? goalId : undefined, targetCount: type === 'no_spend_days' || type === 'tracking_days' ? value : undefined, targetAmount: type === 'category_limit' || type === 'goal_contribution' ? value : undefined, startsOn: localDateKey(), endsOn: localDateKey(ends), status: 'active', createdAt: new Date().toISOString() }); onClose() }
