...
export function DailyReminderSetting() {
  ...
  return <div className="daily-reminder-setting">
    ...
    {(note || reminderSupport()) && <p className="daily-reminder-status vault-sheet-note" role="status">{note || reminderSupport()}</p>}
    {notificationSettingsNeeded && <div className="daily-reminder-status"><button className="vault-link" type="button" onClick={() => { void openNativeNotificationSettings() }}>Open Android notification settings</button></div>}
  </div>
}
