...
import { WeeklyRecapLink } from '../WeeklyRecap'
...
import { calculateIncomeCycle, detectMoneyLeak } from '../../utils/journeyCalculations'
...
const transactionTone = (transaction: Transaction) => (transaction.type === 'income' || transaction.type === 'receivable_payment') ? 'income' : transaction.type === 'transfer' ? 'moved' : 'spent'
...
function periodTransactions(data: DesktopFinanceData, period: PeriodKey) {
  ...
  if (period === 'cycle') {
    const cycle = calculateIncomeCycle(data.journeySettings)
    if (cycle) return data.transactions.filter((item) => item.date >= cycle.startDate && item.date <= cycle.endDate)
  }
  ...
  const results = useMemo(() => {
    ...
    const items: PaletteResult[] = [
      ...
      ...sortedTransactions(transactions).slice(0, 20).map((transaction) => ({
        ...
        detail: `${transactionCategory(transaction)} · ${transaction.account} · ${money(transaction.amount)}`,
...
function Topbar({ page, openModal, openPalette, attentionCount, setActivePage, period, setPeriod, accountFilter, setAccountFilter }: { page: DesktopPage; openModal: OpenModal; openPalette: () => void; attentionCount: number; setActivePage: (page: string) => void; period: PeriodKey; setPeriod: (period: PeriodKey) => void; accountFilter: string; setAccountFilter: (accountId: string) => void }) {
  const { accounts, journeySettings, profile } = useDesktopData()
  ...
  const cycle = calculateIncomeCycle(journeySettings)
  const eyebrow = page === 'dashboard'
    ...
    : page === 'settings'
      ...
      : page === 'categories'
        ? 'Settings · Categories'
        : `${cycleDateLabel(cycle)} · Current cycle`
  return <header className="d-topbar">
    ...
    <div className="d-top-actions">
      ...
      {page === 'goals' && <Button onClick={() => openModal('path')}><Plus size={17} />New path</Button>}
...
function HomePage({ setActivePage, openModal }: { setActivePage: (page: string) => void; openModal: OpenModal }) {
  const { accounts, budgets, goals, journeySettings, transactions, upcomingExpenses, wishlistItems } = useDesktopData()
  ...
  return <div className="d-columns">
    <div className="d-work d-home-work">
      <section className="d-hero d-balance-hero">
        <div><Label>● &nbsp; Total balance</Label><p className="d-hero-number"><small>{currencySymbol()}</small> {nf(balance)}</p><span>Across {accounts.length} {accounts.length === 1 ? 'account' : 'accounts'} · synced</span></div>
      ...
      <div className="d-mini-grid">
        <div className="d-mini-card is-dashed"><Label>Cycle</Label><Money>{cycle ? <>Day {cycle.daysElapsed}<small>/{cycle.totalDays}</small></> : 'Setup needed'}</Money><span>● {cycle ? 'Current cycle' : 'Set your income cycle'}</span></div>
        <button type="button" className="d-mini-card" onClick={() => setActivePage('accounts')}><Label>Next income</Label><Money>{cycle ? cycle.daysRemaining : '—'} <small>days</small></Money><span>Open wallet <b>→</b></span></button>
      </div>
    ...
    <aside className="d-attention">
      ...
      <section className="d-leak"><div><Label>{leak ? 'Money leak found' : 'Spending signal'}</Label><span>↗</span></div><h2>{leak?.title ?? 'Nothing unusual yet'}</h2><Money>{leak ? `${money(leak.amount)}` : 'All clear'}</Money><p>{leak ? `${leak.transactionCount} purchases in 30 days — set a limit?` : 'Keep recording and your patterns will appear here.'}</p></section>
...
function LedgerPage({ period, setPeriod, openModal }: { period: PeriodKey; setPeriod: (period: PeriodKey) => void; openModal: OpenModal }) {
  ...
  const filtered = useMemo(() => transactions.filter((item) => {
    ...
    if (filter === 'received') return (item.type === 'income' || item.type === 'receivable_payment')
    if (filter === 'moved') return item.type === 'transfer'
    return item.type === 'expense' || item.type === 'debt_payment' || item.type === 'goal_saving'
  }), [filter, transactions])
  const income = transactions.filter((item) => (item.type === 'income' || item.type === 'receivable_payment')).reduce((sum, item) => sum + item.amount, 0)
...
function WalletPage({ openModal, accountFilter, setAccountFilter }: { openModal: OpenModal; accountFilter: string; setAccountFilter: (accountId: string) => void }) {
  ...
  return <div className="d-columns">
    <div className="d-work">
      <section className="d-hero d-wallet-hero"><div><Label>On hand · {accountFilter === 'all' ? 'all accounts' : visibleAccounts[0]?.name ?? 'account'}</Label><p className="d-hero-number">{money(total)}</p><span><b>{income - spent >= 0 ? '+' : '−'}{nf(Math.abs(income - spent))}</b> net · after {money(spent)} spent</span></div><div><Button onClick={() => openModal('move')}><ArrowRight size={17} />Move money</Button><Button kind="quiet" onClick={() => openModal('account')}><Plus size={17} />Add account</Button></div></section>
...
const chartTooltipLabelStyle = { color: '#FFFDF8', fontWeight: 700, marginBottom: 4 }
const chartTooltipItemStyle = { color: '#F3EEE4', fontWeight: 600 }

function InsightsPage({ openModal, period, setPeriod }: { openModal: OpenModal; period: PeriodKey; setPeriod: (period: PeriodKey) => void }) {
  const data = useDesktopData()
  const { categories: financeCategories } = data
  const transactions = useMemo(() => periodTransactions(data, period), [data, period])
...
function SettingsPage({ setActivePage, onSignOut, openModal, onRestartJourney }: { setActivePage: (page: string) => void; onSignOut: () => void; openModal: OpenModal; onAnalyticsConsentChange: (granted: boolean) => void; onRestartJourney: () => void }) {
  ...
  return <div className="d-columns d-settings-grid">
    <div className="d-work">
      ...
      <Card className="d-settings-card d-fill"><Label>Preferences</Label><AndroidDownload /><DailyReminderSetting />{!isNativeApp && <div><p><strong>Install Pocket Ledger</strong><small>Add the app to this device</small></p><button type="button" onClick={requestPwaInstall}>Install</button></div>}<div><p><strong>Private usage analytics</strong><small>{journeySettings.analyticsConsent ? 'On · No email or financial content' : 'Not enabled for this account'}</small></p><strong>{journeySettings.analyticsConsent ? 'On' : 'Off'}</strong></div><div><p><strong>Journey setup</strong><small>{journeySettings.nextIncomeDate ? `Next income ${journeySettings.nextIncomeDate}` : 'Income cycle not configured'}</small></p><button type="button" onClick={onRestartJourney}>Update cycle</button></div></Card>
    </div>
    <aside className="d-attention">
      <Card className="d-settings-card d-fill"><Label>Data & security</Label><button className="d-setting-link" onClick={() => setActivePage('categories')}><span className="d-account-icon is-sage"><WalletCards size={18}/></span><strong>Manage categories</strong><ChevronRight size={15}/></button><button className="d-setting-link" onClick={() => setActivePage('accounts')}><span className="d-account-icon is-blue"><CreditCard size={18}/></span><strong>Linked accounts</strong><small>{accounts.length}</small><ChevronRight size={15}/></button><button className="d-setting-link" onClick={() => { void exportTransactionsCsv(transactions).catch(() => window.alert('Could not export transactions. Please try again.')) }}><span className="d-account-icon is-sand"><Download size={18}/></span><strong>Export data</strong><small>CSV</small><ChevronRight size={15}/></button></Card>
...
const modalCopy: Record<ModalKind, { badge: string; title: string; subtitle: string; save: string }> = {
  ...
  move: { badge: 'Transfer', title: 'Move money', subtitle: 'Shift money between your accounts.', save: `Move ${currencySymbol()} 2,000` },
  ...
  funds: { badge: 'Goal deposit', title: 'Add funds', subtitle: 'Move one of your paths closer.', save: `Add ${currencySymbol()} 2,000` },
  path: { badge: 'New path', title: 'What would you like to track?', subtitle: 'Create a goal or bring a debt repayment into view.', save: 'Create path' },
...
function SlideOver({ kind, targetId, close, setActivePage, onRecordEntry, onMoveMoney, onCreateGoal, onCreateWishlistItem, onCreateAccount, onAddFunds, onCreateBudget, onUpdateTransaction, onDeleteTransaction, onUpdateAccount, onArchiveAccount, onRestoreAccount, onUpdateGoal, onDeleteGoal, onCreateDebt, onUpdateDebt, onDeleteDebt, onPayDebt, onCreateUpcoming, onUpdateUpcoming, onDeleteUpcoming, onPayUpcoming, onUpdateWishlist, onDeleteWishlist, onBuyWishlist, onSaveQuest, onCancelQuest, onSaveCategory, onUpdateProfile }: {
  ...
  const [pathMode, setPathMode] = useState<'choose' | 'goal' | 'debt'>(kind === 'path' ? 'choose' : 'goal')
  ...
  return <div className="d-modal-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
    <aside className="d-slide-over" role="dialog" aria-modal="true" aria-labelledby="desktop-modal-title">
      ...
      {!(kind === 'entry' && selectedTransaction?.type === 'receivable_payment') && kind !== 'search' && kind !== 'notifications' && kind !== 'wishlist' && kind !== 'archivedAccounts' && !(kind === 'plan' && planMode === 'choose') && !(kind === 'path' && pathMode === 'choose') && <footer><Button disabled={(kind === 'record' && (!amount || !entryCategory || !accountId || !date)) || (kind === 'entry' && entryMode === 'edit' && (!selectedTransaction || !entryTitle.trim() || amount <= 0 || !date)) || (kind === 'move' && (!amount || !accountId || !toAccountId || accountId === toAccountId || amount > (fromAccount?.balance ?? 0))) || (kind === 'cooloff' && (!itemName.trim() || amount <= 0)) || ((kind === 'goal' || (kind === 'path' && pathMode === 'goal')) && (!goalName.trim() || amount <= 0)) || (kind === 'goalManage' && (!selectedGoal || !goalName.trim() || amount <= 0)) || ((kind === 'debt' || kind === 'debtManage' || (kind === 'path' && pathMode === 'debt')) && (!debtName.trim() || amount <= 0)) || (kind === 'debtPayment' && (!selectedDebt || !accountId || amount <= 0 || amount > Math.max(0, debtTotal(selectedDebt) - debtPaid(selectedDebt)) || amount > (fromAccount?.balance ?? 0))) || (kind === 'account' && !accountName.trim()) || (kind === 'accountManage' && (!selectedAccount || !accountName.trim())) || (kind === 'category' && !categoryName.trim()) || (kind === 'funds' && (!goalId || !accountId || amount <= 0)) || (kind === 'plan' && planMode === 'limit' && (!budgetCategory || amount <= 0)) || (kind === 'plan' && planMode === 'bill' && (!billTitle.trim() || !budgetCategory || !dueDate || amount <= 0)) || (kind === 'plan' && planMode === 'cooloff' && (!itemName.trim() || amount <= 0)) || (kind === 'plan' && planMode === 'quest' && amount <= 0) || (kind === 'billManage' && (!billTitle.trim() || !budgetCategory || !dueDate || amount <= 0)) || (kind === 'billPayment' && (!selectedBill || !accountId || !date || (fromAccount?.balance ?? 0) < (selectedBill?.amount ?? 0))) || (kind === 'quest' && amount <= 0) || (kind === 'profile' && !profileName.trim())} onClick={() => {
        ...
        if (kind === 'record') onRecordEntry({ direction, amount, category: entryCategory, accountId, date, notes: notes.trim() || undefined })
        ...
        else if (kind === 'goal' || (kind === 'path' && pathMode === 'goal')) onCreateGoal({ name: goalName.trim(), target: amount, dueDate: dueDate || undefined, notes: notes.trim() || undefined })
        ...
        else if (kind === 'debt' || kind === 'debtManage' || (kind === 'path' && pathMode === 'debt')) {
          ...
          onSaveQuest({ id: `quest-${Date.now().toString(36)}`, type: questType, title: questType === 'no_spend_days' ? `${amount} no-spend days` : questType === 'tracking_days' ? `Track every spend for ${amount} days` : `Keep ${budgetCategory} under ${money(amount)}`, categoryId: questType === 'category_limit' ? categories.find((item) => item.name === budgetCategory)?.id : undefined, targetAmount: questType === 'category_limit' ? amount : undefined, targetCount: questType === 'category_limit' ? undefined : amount, startsOn, endsOn: localDateKey(ends), status: 'active' })
        }
        ...
        close()
      }}>{kind === 'entry' && entryMode === 'view' ? <><Pencil size={17}/>Edit entry</> : <><Check size={18}/>{kind === 'path' && pathMode === 'goal' ? 'Create goal' : kind === 'path' && pathMode === 'debt' ? 'Add debt' : copy.save}</>}</Button></footer>}
    </aside>
  </div>
}
...
export function DesktopExperience({ activePage, setActivePage, data, onSignOut, onRecordEntry, onMoveMoney, onCreateGoal, onCreateWishlistItem, onCreateAccount, onAddFunds, onCreateBudget, onSaveBudget, onArchiveBudget, onRestoreBudget, onCopyLastMonthBudgets, onUpdateTransaction, onDeleteTransaction, onUpdateAccount, onArchiveAccount, onRestoreAccount, onUpdateGoal, onDeleteGoal, onCreateDebt, onUpdateDebt, onDeleteDebt, onPayDebt, onCreateUpcoming, onUpdateUpcoming, onDeleteUpcoming, onPayUpcoming, onUpdateWishlist, onDeleteWishlist, onBuyWishlist, onSaveQuest, onCancelQuest, onSaveCategory, onArchiveCategory, onRestartJourney, onUpdateProfile, onAnalyticsConsentChange }: DesktopExperienceProps) {
  ...
  useBackDismiss(Boolean(modal), closeModal)
  useBackDismiss(paletteOpen, () => setPaletteOpen(false))
  ...
  return <DesktopDataContext.Provider value={data}><div className="desktop-experience">
      ...
      <main className="d-main">
        ...
        {page === 'goals' && <div><a className="vault-chip mb-4 inline-flex" href="#receivables">Owed to me ↗</a><GoalsDesktop goals={data.goals} debts={data.debts} accounts={data.accounts} onNewPath={() => openModal('path')} onAddFunds={(goalId) => openModal('funds', goalId)} onEditGoal={(goalId) => openModal('goalManage', goalId)} onCompleteGoal={(goal) => onUpdateGoal(goal.id, { name: goal.name, target: goal.target, saved: goal.saved, dueDate: goal.dueDate, linkedAccountId: goal.linkedAccountId, notes: goal.notes, status: 'Completed' })} onDeleteGoal={onDeleteGoal} onPayDebt={(debtId) => openModal('debtPayment', debtId)} onAddDebt={() => openModal('debt')} onEditDebt={(debtId) => openModal('debtManage', debtId)} onDeleteDebt={onDeleteDebt}/><Receivables accounts={data.accounts} /></div>}
