    ...
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)]">
      ...
      <div className="relative flex min-h-screen">
        ...
        <main className="w-full pb-[calc(7.75rem+env(safe-area-inset-bottom))] lg:pb-0">
          <header className={`${isDashboard ? 'hidden' : 'sticky'} top-0 z-20 border-b border-white/8 bg-[var(--bg-deep)]/88 px-4 py-3 backdrop-blur-xl sm:px-6 sm:py-4 lg:px-8`}>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activePage} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className={`mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 ${isDashboard ? 'py-0 sm:py-5' : 'py-4 sm:py-5'}`}>
              {children}
            </motion.div>
          </AnimatePresence>
        </main>
